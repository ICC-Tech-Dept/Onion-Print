...
def dual_expire_test():
    global val
    if time.time() > (val['dual_submit_time'] + 300) and val['status'] == 4:
        logging.info('TIMEOUT - user payment timeout')
...
def qr_send():
    ...
    if val['user_requests'] != [] and val['status'] == 0 and val['user_requests'][0][5] != 0 and val['user_requests'][0][2] == 0:
        ...
        itchat.send(msg='文件计算后的价格为%.1f元\n请在45秒内扫描下方的二维码唷' % price, toUserName=val['user_requests'][0][0])
        itchat.send(msg='可以在扫码前发送"双面"以双面打印，或"彩色"以彩色打印', toUserName=val['user_requests'][0][0])
        itchat.send('@img@QRs/%.1f.jpg' % price, toUserName=val['user_requests'][0][0])
        ...
        if pages > 50:
            ...
            if len(val['user_requests']) != 0:
                ...
                if Time > 60:
                    if Time % 60 == 0:
                        minute = Time // 60
                        itchat.send(msg=f'您的文件已接收，但是前面有人在排队哦\n您无需重发文件，大约{minute}分钟后我会为您出示付款码，继续操作~', toUserName=msg.fromUserName)
                    else:
                        minute = Time // 60
                        second = Time % 60
                        itchat.send(msg=f'您的文件已接收，但是前面有人在排队哦\n您无需重发文件，大约{minute}分{second}秒后我会为您出示付款码，继续操作~', toUserName=msg.fromUserName)
                else:
                    itchat.send(msg=f'您的文件已接收，但是前面有人在排队哦\n您无需重发文件，大约{Time}秒后我会为您出示付款码，继续操作~', toUserName=msg.fromUserName)
            color = 0
            number = 0
            val['user_requests'].append((msg.fromUserName, filename, price, color, orientation, number))
        ...
        if price >= round(val['user_requests'][0][2], 2):
            ...
            if val['status'] == 1:
                ...
                if split_file():
                    ...
                    val['user_requests'][0] = (val['user_requests'][0][0], val['user_requests'][0][1], val['user_requests'][0][2], val['user_requests'][0][3], val['user_requests'][0][4], val['user_requests'][0][5] - 1)
...
def receive_cancel_message(msg):
    ...
    if msg.text == '双面' and val['status'] == 1 and msg.fromUserName == val['user_requests'][0][0]:
        itchat.send('已收到双面打印请求\n请在45秒内扫描二维码', toUserName=msg.fromUserName)
        itchat.send(msg='可以在扫码前发送"彩色"以彩色打印', toUserName=val['user_requests'][0][0])
        val['submit_time'] = time.time()
        val['status'] = 2
    if msg.text == '彩色' and (val['status'] == 1 or val['status'] == 2) and msg.fromUserName == val['user_requests'][0][0]:
        itchat.send('已收到彩色打印请求，请稍等', toUserName=msg.fromUserName)
        pages = PdfFileReader(val['user_requests'][0][1]).getNumPages()
        price = pages * val['user_requests'][0][5] * 0.5
        val['user_requests'][0] = (val['user_requests'][0][0], val['user_requests'][0][1], price, 1, val['user_requests'][0][4], val['user_requests'][0][5])
        ...
        val['submit_time'] = time.time()
        win32print.SetDefaultPrinter(val['color_printer'])
    if msg.text == '继续' and val['status'] == 4 and msg.fromUserName == val['user_requests'][0][0]:
        ...
        if val['user_requests'][0][5] == 0:
            ...
            itchat.send('请在前一份打印完成后完全拿走\n然后发送"完成"', toUserName=val['user_requests'][0][0])
    if msg.text == '完成' and val['status'] == 4 and msg.fromUserName == val['user_requests'][0][0]:
        val['dual_submit_time'] = time.time()
        itchat.send('打印正面中.....\n待第一面打印完成后，将打印出的纸张直接放到下方纸摞上\n！注意不要改变纸张方向\n放好后发送"继续"以打印反面', toUserName=val['user_requests'][0][0])
        win32api.ShellExecute(0, 'print', (val['user_requests'][0][1])[:-4] + 'a.pdf', '/d:"%s"' % win32print.GetDefaultPrinter(), ".", 0)
        val['user_requests'][0] = (val['user_requests'][0][0], val['user_requests'][0][1], val['user_requests'][0][2], val['user_requests'][0][3], val['user_requests'][0][4], val['user_requests'][0][5] - 1)
