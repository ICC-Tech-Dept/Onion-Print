        ...
        if pages > 50:
            ...
            if len(val['user_requests']) != 0:
                ...
                if Time > 60:
                    if Time % 60 == 0:
                        minute = Time // 60
                        itchat.send(msg=f'有其他人正在支付，可能需要稍等{minute}分钟噢', toUserName=msg.fromUserName)
                    else:
                        minute = Time // 60
                        second = Time % 60
...
def receive_cancel_message(msg):
    ...
    if msg.text == '继续' and val['status'] == 4 and msg.fromUserName == val['user_requests'][0][0]:
        ...
        if val['user_requests'][0][5] == 0:
            ...
            itchat.send('请在前一份打印完成后完全拿走，然后发送“完成”', toUserName=val['user_requests'][0][0])
